    ...
    methods: {
        ...
        newNoteCreation() {
            axios.post("http://localhost:8000/api/notes", {
                ...
                .then((response) => {
                    useOpenNotification({ id: 'creata-nota', bgType: 'bg-success', message: 'Nota creata con successo' });
        ...
        noteUpdate(formId, noteId) {
            axios.put("http://localhost:8000/api/notes/" + noteId, {
                ...
                .then((response) => {
                    useOpenNotification({ id: 'update-nota-' + noteId, bgType: 'bg-success', message: 'Nota modificata con successo' });
        ...
        deleteNote(noteId) {
            ...
            axios.delete("http://localhost:8000/api/notes/" + noteId)
                .then((response) => {
                    useOpenNotification({ id: 'eliminata-nota-' + noteId, bgType: 'bg-success', message: 'Nota eliminata con successo' });
                ...
                <li v-for="singleNote in notesData" class="collapse collapse-arrow shadow-md bg-white mb-2">
                    ...
                    <div class="collapse-content flex justify-between">
                        ...
                        <div>
                            ...
                            <button v-on:click="updateAccordionToggle(singleNote.id)"
                                class="btn btn-warning btn-sm btn-square">
                                <font-awesome-icon v-if="updateCollapseFlag === false"
                                    :icon="['fas', 'pen-to-square']" /> <!-- Icona con il collapse chiuso-->
                                <font-awesome-icon v-else :icon="['fas', 'xmark']" />
                                <!-- Icona con il collapse aperto-->
                            </button>

                            <!-- Pulsante di elimina nota -->
                            <button class="btn btn-error btn-sm btn-square ms-2"
                                v-on:click="openOrCloseModal('delete_note_' + singleNote.id + '_modal')">
                            ...
                            <dialog :id="'delete_note_' + singleNote.id + '_modal'" class="modal">
                                <div class="modal-box">
                                    <h3 class="font-bold text-lg">Sicuro di voler eliminare la nota "{{ singleNote.title
                                        }}"?</h3>



                                    <div class="modal-action">
                                        <button class="btn btn-error ms-2"
                                            v-on:click="deleteNote(singleNote.id)">Conferma</button>
                    ...
                    <div :id="singleNote.id" class="collapse collapse-close collapse_update">
                        <form :id="'update_note_' + singleNote.id"
                            class="collapse-content update_note_form flex flex-col">
                            <input v-model="formUpdateNotes.title" type="text" placeholder="Nuovo titolo"
                                class="input input-bordered w-full" name="title" minlength="1" maxlength="50"
                                required />

                            <textarea v-model="formUpdateNotes.noteBody" class="textarea textarea-bordered my-4"
                                placeholder="Nuovo testo della nota" name="noteBody" required></textarea>

                            <input v-on:click.prevent="noteUpdate('update_note_' + singleNote.id, singleNote.id)"
                                type="submit" value="Invia" class="btn btn-success self-end">
                        </form>
                    </div>
                </li>
            </ul>
            ...
            <dialog id="insert_note_modal" class="modal">
                <div class="modal-box">
                    ...
                    <form class="new_note_form flex flex-col my-4">
                        <input v-model="formStoreNotes.title" type="text" placeholder="Titolo"
                            class="input input-bordered w-full" name="title" minlength="1" maxlength="50" required />

                        <textarea v-model="formStoreNotes.noteBody" class="textarea textarea-bordered my-4"
                            placeholder="Testo della nota" name="noteBody" required></textarea>
